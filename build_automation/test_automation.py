...
for i in range(0, int(numProblems)): #for each problem
	for j in range(0, int(numAgents)): #for each agent
		procCommand = "gnome-terminal -x ssh pi@" + piips[j] + " java -cp " + classpath + " smartgrids.AgentGenerator " + "/home/pi/4_agents/test/agents/p_" + str(i) + filename_endings + "/a_" + str(j) + ".xml"   
		#print(procCommand)
